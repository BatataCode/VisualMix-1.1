...
if (!blocosSemBotao.includes(tipo)) {
  ...
  bloco.innerHTML += `
    <h3 style="display:flex; justify-content:space-between; align-items:center;">
      ...
      <button class="toggle-visibilidade" onclick="alternarVisibilidadeFilhos(this)">visível</button>
    </h3>
  ...
  bloco.innerHTML += `
    ...
    <input type="text" placeholder="URL da imagem" class="valor-radius" />
  `;
} else if (tipo === 'camera') {
...
function removerBloco(botao) {
  modoCopiaAtivo = false;
  let botaoCopiar = document.getElementById("botaoCopiarBloco");
  botaoCopiar.classList.remove("ativo");
...
function carregarBlocos() {
  ...
  dados.forEach(({ tipo, idElemento, valor, condicao, nomeVariavel, valorVariavel, larguraElemento, alturaElemento, posicaoX, posicaoY, nomeLista, valoresLista, comentarioOculto }) => {
  ...
  if (botao) {
    // Chama a função que alterna visibilidade, mas forçando ocultar os filhos
...
for (let i = 0; i < blocos.length; i++) {
  ...
  if (bloco.classList.contains("bloco-comentario") && bloco.classList.contains("comentario-oculto")) {
    ...
    if (botao) botao.innerText = "oculto";
    }
  }
}
...
function aplicarVisibilidadeConformeEstado(blocoComentario) {
  ...
  if (oculto) {
    ...
    if (botao) botao.innerText = "visível";
  }
